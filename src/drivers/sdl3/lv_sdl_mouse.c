...
#if LV_SDL3_MOUSEWHEEL_MODE == LV_SDL_MOUSEWHEEL_MODE_CROWN
    int32_t diff;
#endif
} lv_sdl_mouse_t;
...
void lv_sdl_mouse_handler(SDL_Event * event)
{
    ...
    switch(event->type) {
        ...
        case SDL_FINGERMOTION:
            ...
            break;
        /*SDL3 removed the SDL_WINDOWEVENT parent type; SDL_WINDOWEVENT_LEAVE
          is now a top-level event type (mapped via OLD_NAMES).*/
    ...
    float input_scale = lv_sdl_window_get_input_scale(disp);
    ...
    switch(event->type) {
        ...
        case SDL_MOUSEBUTTONDOWN:
            if(event->button.button == SDL_BUTTON_LEFT) {
                ...
                indev_dev->last_x = (int16_t)((float)(event->motion.x) * input_scale / zoom);
                indev_dev->last_y = (int16_t)((float)(event->motion.y) * input_scale / zoom);
            }
            break;
        case SDL_MOUSEMOTION:
            indev_dev->last_x = (int16_t)((float)(event->motion.x) * input_scale / zoom);
        ...
        case SDL_MOUSEWHEEL:
            ...
            if(event->wheel.y > 0) dsc->diff--;
#else
            /*SDL3 changed event.wheel.y to float; cast preserves truncation.*/
            indev_dev->diff = -(int32_t)event->wheel.y;
#endif  /*__EMSCRIPTEN__*/
#endif /*LV_SDL3_MOUSEWHEEL_MODE == LV_SDL_MOUSEWHEEL_MODE_CROWN*/
            break;
    }
    lv_indev_read(indev);
}
