...
#if LV_USE_SDL3 && !LV_USE_DRAW_SDL
...
static lv_result_t init_display(lv_display_t * display)
{
    ...
    ddata->renderer = SDL_CreateRenderer(lv_sdl_window_get_window(display),
                                         LV_SDL3_ACCELERATED ? NULL : "software");
    if(!ddata->renderer) {
    ...
    if(LV_SDL3_RENDER_MODE == LV_DISPLAY_RENDER_MODE_PARTIAL) {
        ...
        lv_display_set_buffers(display, ddata->buf1, ddata->buf2, buffer_size_bytes, LV_SDL3_RENDER_MODE);
    }
    else {
...
static lv_result_t resize_display(lv_display_t * display)
{
    ...
    if(sdl_render_mode() == LV_DISPLAY_RENDER_MODE_PARTIAL) {
        ...
        lv_display_set_buffers(display, ddata->fb1, ddata->fb2, stride * display->ver_res, LV_SDL3_RENDER_MODE);
    }
...
#endif /*LV_USE_SDL3 && !LV_USE_DRAW_SDL*/
